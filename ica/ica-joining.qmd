...
It didn't give any information regarding the enrolments, so it had the left most data and things that matched (so student C is not present) but it did not have any of the right most data

-   How does `semi_join()` differ from `inner_join()`?
Inner join includes the right most data from the matching variables

-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
No student column. Class stat and geo and then their enrollment 
...
-   What did this do? What info would it give us?
Gave the one piece of data that didn't match from the left most column
